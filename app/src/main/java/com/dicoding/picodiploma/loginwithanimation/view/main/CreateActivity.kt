...
class CreateActivity : AppCompatActivity() {
    ...
    private val launcherCamera = registerForActivityResult(
        ...
        if (success) {
            currentPhotoUri?.let { uri ->
                ...
                } ?: Toast.makeText(this, "Gagal membaca URI gambar.", Toast.LENGTH_SHORT).show()
            }
        } else {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val toolbar: Toolbar = findViewById(R.id.toolbarcreate)
                        ...
                        if (addLocationCheckBox.isChecked) {
                            if (isLocationPermissionGranted()) {
                                ...
                                if (location != null) {
                                    ...
                                    Toast.makeText(this@CreateActivity, "Lokasi tidak ditemukan.", Toast.LENGTH_SHORT).show()
                                }
                            } else {
    ...
    private suspend fun getCurrentLocationAsync(): android.location.Location? {
        if (ContextCompat.checkSelfPermission(
            ...
            return try {
                ...
                Toast.makeText(this, "Gagal mendapatkan lokasi: ${e.message}", Toast.LENGTH_SHORT)
                    .show()
                null
            }
        } else {
        ...
        return try {
            suspendCancellableCoroutine { continuation ->
                ...
                fusedLocationClient.requestLocationUpdates(
                    ...
                    continuation.resume(null) {}
                    Toast.makeText(this, "Gagal memulai pembaruan lokasi: ${e.message}", Toast.LENGTH_SHORT).show()
                }

                // Jika pembatalan coroutine terjadi, pastikan callback dihentikan
                continuation.invokeOnCancellation {
                    fusedLocationClient.removeLocationUpdates(locationCallback)
                }
            }
        } catch (e: SecurityException) {
            Toast.makeText(this, "Izin lokasi tidak tersedia: ${e.message}", Toast.LENGTH_SHORT).show()
            null
        } catch (e: Exception) {
            Toast.makeText(this, "Gagal mendapatkan lokasi baru: ${e.message}", Toast.LENGTH_SHORT).show()
            null
        }
    }
